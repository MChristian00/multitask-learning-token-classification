...
def train_sequential_model(first_task, second_task):
    ...
    if first_task not in ['pos', 'lid'] or second_task not in ['pos', 'lid'] or first_task == second_task:
        raise ValueError(
            "first_task and second_task must be 'pos' or 'lid' and different.")

    print(
        f"Starting Sequential Training: {first_task.upper()} then {second_task.upper()}\n\n")

    # Train on First Task
    print(f"\nStarting Phase 1: Training on {first_task.upper()}...")

    # Load and preprocess data for the first task
    train_dataset_first, dev_dataset_first, label2id_first, id2tag_first = load_and_preprocess_data(
        first_task)
    ...
    output_dir_first = os.path.join(
        SAVES_DIR, f'sequential_phase1_{first_task}')
    ...
    trainer_first = Trainer(
        ...
        compute_metrics=lambda p: compute_single_task_metrics(
            p, id2tag_first)  # Metrics for the first task
    )
    ...
    phase1_save_path = os.path.join(
        MODEL_SAVE_DIR, f'sequential_phase1_{first_task}-final')
    ...
    print(
        f"Phase 1 ({first_task.upper()}) training complete. Model saved to {phase1_save_path}")

    # Fine-tune on Second Task
    print(f"\nStarting Phase 2: Fine-tuning on {second_task.upper()}...")

    # Load and preprocess data for the second task
    train_dataset_second, dev_dataset_second, label2id_second, id2tag_second = load_and_preprocess_data(
        second_task)
    ...
    output_dir_second = os.path.join(
        SAVES_DIR, f'sequential_phase2_{second_task}')
    ...
    trainer_second = Trainer(
        ...
        data_collator=data_collator,
        compute_metrics=lambda p: compute_single_task_metrics(
            p, id2tag_second)  # Metrics for the second task
    )
    ...
    print(
        f"Evaluating sequential ({first_task.upper()} then {second_task.upper()}) model on development set...")
    eval_results = trainer_second.evaluate(dev_dataset_second)
    print("\n\n\n ##########################################")
    print(
        f"Sequential ({first_task.upper()} then {second_task.upper()}) Evaluation Results:", eval_results)
    print("##########################################\n\n\n")
    ...
    print(
        f"--- Sequential Training ({first_task.upper()} then {second_task.upper()}) Complete ---")
