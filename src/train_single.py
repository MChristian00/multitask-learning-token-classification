...
def train_single_task_model(task_type):
    ...
    train_dataset, dev_dataset, label2id, id2tag = load_and_preprocess_data(
        task_type)
    ...
    args = TrainingArguments(
        ...
        report_to="none",
        # Uses fp16 if GPU is available for faster training and lower memory usage
    ...
    trainer = Trainer(
        ...
        compute_metrics=lambda p: compute_single_task_metrics(
            p, id2tag)  # Pass id2tag to metrics function
    )
    ...
    print(
        f"Evaluating single-task {task_type.upper()} model on development set...")
    ...
    print(
        f"Saving fine-tuned single-task {task_type.upper()} model to {model_save_path}")
