        ...
        with open(filepath, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if task_type == 'pos' and len(parts) >= 3:
                    # Assuming token\tlang\tpos
                    token, _, pos = parts[0], parts[1], parts[2]
                    tokens.append(token)
                    tags.append(pos)
                    ...
                    # Assuming token\tlang\tpos
                    token, lid, pos = parts[0], parts[1], parts[2]
                    tokens.append(token)
                    lid_tags.append(lid)
                    pos_tags.append(pos)
                # else:
...
def tokenize_and_align_single_task(batch, label2id, tag_key):
    ...
    tokens = batch["tokens"]
    tags = batch[tag_key]
    labels = [[label2id[tag] for tag in sentence_tags]
...
def tokenize_and_align_multitask(batch, lid2id, pos2id):
    ...
    lid_labels = [[lid2id[tag] for tag in sentence_lid_tags]
                  for sentence_lid_tags in lid_tags]
    pos_labels = [[pos2id[tag] for tag in sentence_pos_tags]
...
def load_and_preprocess_data(task_type):
    ...
    if task_type == 'pos':
        ...
        train_dataset = Dataset.from_list(train_data).map(
            lambda batch: tokenize_and_align_single_task(
                batch, pos2id, 'pos_tags'),
