...
class MultitaskTokenClassification(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if classifier_dropout is None:
            # 0.1 default fallback dropout value
            classifier_dropout = getattr(config, 'hidden_dropout_prob', 0.1)

        self.dropout = nn.Dropout(classifier_dropout)

        # Separate classification heads for each task
        self.lid_classifier = nn.Linear(
            config.hidden_size, self.num_labels_lid)
        self.pos_classifier = nn.Linear(
            config.hidden_size, self.num_labels_pos)
    ...
    def forward(
        ...
        **kwargs  # for any addt'l arguments
        ...
        if labels_lid is not None and labels_pos is not None:
            # Ignore index for padded tokens
            loss_fct = nn.CrossEntropyLoss(ignore_index=-100)

            # Calculate loss for each task
            loss_lid = loss_fct(
                lid_logits.view(-1, self.num_labels_lid), labels_lid.view(-1))
            loss_pos = loss_fct(
                pos_logits.view(-1, self.num_labels_pos), labels_pos.view(-1))

            # Combine losses
            if (UNCERTAINTY_WEIGHTING):
                # Uncertainty-Based Weighting (Kendall et al., 2018)
                # Use the log variance parameters to weight the losses
                pos_term = torch.exp(-self.log_var_pos) * \
                    loss_pos + self.log_var_pos
                lid_term = torch.exp(-self.log_var_lid) * \
                    loss_lid + self.log_var_lid
