...
def compute_single_task_metrics(p, id2tag):
    ...
    true_labels = [
        [id2tag[label]
            for (p, label) in zip(prediction, labels_per_sent) if label != -100]
...
def compute_multitask_metrics(p, id2lid, id2pos):
    ...
    if lid_labels is not None:
        ...
        true_lid_labels = [
            [id2lid[label] for (pred, label) in zip(
                prediction, labels_per_sent) if label != -100]
            for prediction, labels_per_sent in zip(lid_predicted_label_ids, lid_labels)
        ]
        true_lid_predictions = [
            [id2lid[pred] for (pred, label) in zip(
                prediction, labels_per_sent) if label != -100]
            for prediction, labels_per_sent in zip(lid_predicted_label_ids, lid_labels)
        ]

        if true_lid_labels and true_lid_predictions:  # Only compute if there are actual labels
            lid_results = metric.compute(
                predictions=true_lid_predictions, references=true_lid_labels)
            metrics.update({
    ...
    if pos_labels is not None and np.any(pos_labels != -100):
        ...
        if true_pos_labels and true_pos_predictions:
            ...
            print(
                "Warning: No valid POS labels found for metric computation in this batch.")

    # Compute combined F1 if both are available
    if "eval_lid_f1" in metrics and "eval_pos_f1" in metrics:
        metrics["eval_combined_f1"] = (
            metrics["eval_lid_f1"] + metrics["eval_pos_f1"]) / 2.0  # Average of LID and POS F1
    elif "eval_lid_f1" in metrics:
        # If only LID is available, use LID F1
        metrics["eval_combined_f1"] = metrics["eval_lid_f1"]
    elif "eval_pos_f1" in metrics:
        # If only POS is available, use POS F1
        metrics["eval_combined_f1"] = metrics["eval_pos_f1"]
